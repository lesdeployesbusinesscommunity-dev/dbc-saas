    ...
    <section ref={ref} className="mx-auto max-w-6xl px-4 pb-16 sm:px-6 lg:px-8">
      <div className="relative rounded-xl border-2 border-[#52A2DF]/[0.32] p-6 sm:p-8">
        ...
        {/* Widget flottant, à l'extrême opposé du titre (coin bas-droit) :
            une flèche oblique qui, au survol OU au tap (tablette/mobile),
            ouvre une bulle de "chat" contenant le bouton "Rejoins la DBC".
            La flèche est un vrai <button> (plutôt qu'un <div>) : sur
            tactile il n'y a pas de survol, donc un tap doit pouvoir
            déclencher la bulle — "focus" (group-focus-within) fait ce
            travail, comme pour les icônes d'award de l'espace admin. */}
        <div className="group absolute -bottom-5 -right-5 z-10">
          ...
          <div className="pointer-events-none absolute bottom-full right-0 w-56 pb-3 opacity-0 transition-opacity duration-200 ease-out group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100">
            <div className="origin-bottom-right scale-95 rounded-xl border border-gray-200 bg-white p-4 shadow-xl transition-transform duration-200 ease-out group-hover:scale-100 group-focus-within:scale-100">
          ...
          <button
            type="button"
            aria-label={t("visiteur.profiles.widgetPrompt")}
            className="flex size-12 items-center justify-center rounded-full border-2 border-[#52A2DF] bg-white text-[#52A2DF] shadow-md outline-none transition-transform duration-300 ease-out hover:scale-110 hover:bg-[#52A2DF] hover:text-white focus-visible:scale-110 focus-visible:bg-[#52A2DF] focus-visible:text-white"
          >
