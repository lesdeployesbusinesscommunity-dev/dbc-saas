...
function PillarCard({ pillar, isInView, delay }) {
  ...
  return (
    // "group relative z-0 hover:z-30 focus-within:z-30" : le survol
    // applique aussi "hover:-translate-y-1" (un transform), et un
    // transform actif crée un nouveau contexte d'empilement CSS. Sans
    // z-index explicite ici, la carte survolée (avec son popover qui
    // déborde) pouvait se retrouver peinte SOUS la carte suivante de la
    // grille (qui vient après dans le DOM). "hover:z-30" fait passer
    // toute la carte au-dessus de ses voisines pendant le survol —
    // "focus-within:z-30" fait pareil au tap (tablette/mobile, pas de
    // survol). "tabIndex={0}" rend la carte elle-même focusable : sur
    // tactile, taper dessus lui donne le focus, ce qui déclenche le
    // popover via "group-focus-within" plus bas (même logique que les
    // icônes d'award de l'espace admin).
    <div
      tabIndex={0}
      className={clsx(
        "group relative z-0 rounded-xl border border-[#52A2DF]/[0.32] bg-white outline-none transition-all duration-700 ease-out hover:z-30 focus-within:z-30",
        "hover:-translate-y-1 hover:border-[#52A2DF] hover:shadow-lg focus-visible:-translate-y-1 focus-visible:border-[#52A2DF] focus-visible:shadow-lg",
    ...
    >
      ...
      {/* Popover au survol OU au focus (tap sur tablette/mobile, voir
          tabIndex sur la carte plus haut) : reprend le même titre/texte
          que la carte, mais avec l'image complète (object-contain = rien
          n'est coupé), sur un fond dégradé pour que ça reste soigné même
          si l'image ne remplit pas tout le cadre. pointer-events-none/
          auto : invisible et inerte tant qu'on ne survole/focus pas, pour
          ne pas gêner le survol des cartes voisines. */}
      <div
        className={clsx(
